...
/**
 * \brief Loads DAQMapping and AnalysisMask from two XML files.
 **/
class DAQMappingSourceXML: public edm::ESProducer, public edm::EventSetupRecordIntervalFinder
{
public:
  ...
  DAQMappingSourceXML(const edm::ParameterSet &);
  ~DAQMappingSourceXML();

  edm::ESProducts< boost::shared_ptr<DAQMapping>, boost::shared_ptr<AnalysisMask> > produce( const TotemDAQMappingRecord & );
...
private:
  ...
  /// parses XML file
  void ParseXML(ParseType, const string &file, const boost::shared_ptr<DAQMapping>&, const boost::shared_ptr<AnalysisMask>&);
  ...
  void ParseTreeRP(ParseType, xercesc::DOMNode *, NodeType, unsigned int parentID,
    const boost::shared_ptr<DAQMapping>&, const boost::shared_ptr<AnalysisMask>&);

  /// recursive method to extract RP-related information from the DOM tree
  void ParseTreeT1(ParseType, xercesc::DOMNode *, NodeType, unsigned int parentID,
    const boost::shared_ptr<DAQMapping>&, const boost::shared_ptr<AnalysisMask>&, unsigned int T1Arm, unsigned int T1Plane, unsigned int T1CSC);
...
const string DAQMappingSourceXML::tagVFAT="vfat";
const string DAQMappingSourceXML::tagChannel="channel";
const string DAQMappingSourceXML::tagAnalysisMask="analysisMask";

// common XML position tags
const string DAQMappingSourceXML::tagArm = "arm";

// common XML Chip tags
const string DAQMappingSourceXML::tagChip1 = "vfat";
const string DAQMappingSourceXML::tagChip2 = "test_vfat";
const string DAQMappingSourceXML::tagTriggerVFAT1 = "trigger_vfat";

// specific RP XML tags
const string DAQMappingSourceXML::tagRPStation = "station";
const string DAQMappingSourceXML::tagRPPot = "rp_detector_set";
const string DAQMappingSourceXML::tagRPPlane = "rp_plane";

// specific T2 XML tags
const string DAQMappingSourceXML::tagT2="t2_detector_set";
const string DAQMappingSourceXML::tagT2detector="t2_detector";
const string DAQMappingSourceXML::tagT2Half="t2_half";

// specific T1 XML tags
const string DAQMappingSourceXML::tagT1="t1_detector_set";
const string DAQMappingSourceXML::tagT1Arm="t1_arm";
const string DAQMappingSourceXML::tagT1Plane="t1_plane";
const string DAQMappingSourceXML::tagT1CSC="t1_csc";
const string DAQMappingSourceXML::tagT1ChannelType="t1_channel_type";
//----------------------------------------------------------------------------------------------------

DAQMappingSourceXML::DAQMappingSourceXML(const edm::ParameterSet& conf) :
...
{
  try {
    const string &fn = conf.getParameter<string>("xmlMappingFileName");
    if (!fn.empty()) {
      mappingFileNames.push_back(fn);
      printf("!!!!! WARNING in DAQMappingSourceXML: parameter xmlMappingFileName is DEPRECATED, use mappingFileNames instead !!!!!\n");
    }
  }
  catch (...) {
  }
  ...
  try {
    const string &fn = conf.getParameter<string>("xmlMaskFileName");
    if (!fn.empty()) {
      maskFileNames.push_back(fn);
      printf("!!!!! WARNING in DAQMappingSourceXML: parameter xmlMaskName is DEPRECATED, use maskFileNames instead !!!!!\n");
    }
  }
  catch (...) {
  }
  ...
  findingRecord<TotemDAQMappingRecord>();
}

//----------------------------------------------------------------------------------------------------

string DAQMappingSourceXML::CompleteFileName(const string &fn)
{
...
void DAQMappingSourceXML::setIntervalFor(const edm::eventsetup::EventSetupRecordKey&,
...
DOMDocument* DAQMappingSourceXML::GetDOMDocument(string file)
{
  ...
  try {
    ...
    throw cms::Exception("DAQMappingSourceXML::GetDOMDocument") << "Cannot parse file `" << file
      << "' (exception)." << endl;
  }
...
DAQMappingSourceXML::~DAQMappingSourceXML()
{ 
}

//----------------------------------------------------------------------------------------------------

edm::ESProducts< boost::shared_ptr<DAQMapping>, boost::shared_ptr<AnalysisMask> >
  DAQMappingSourceXML::produce( const TotemDAQMappingRecord & )
{
  boost::shared_ptr<DAQMapping> mapping(new DAQMapping());
  boost::shared_ptr<AnalysisMask> mask(new AnalysisMask());
  ...
  try {
    ...
    throw cms::Exception("DAQMappingSourceXML") << "An XMLException caught with message: " << message << ".\n";
...
void DAQMappingSourceXML::ParseXML(ParseType pType, const string &file,
  const boost::shared_ptr<DAQMapping> &mapping, const boost::shared_ptr<AnalysisMask> &mask)
{
  DOMDocument* domDoc = GetDOMDocument(file);
  DOMElement* elementRoot = domDoc->getDocumentElement();

  if (!elementRoot)
    throw cms::Exception("DAQMappingSourceXML::ParseMappingXML") << "File `" <<
      file << "' is empty." << endl;
...
void DAQMappingSourceXML::ParseTreeRP(ParseType pType, xercesc::DOMNode * parent, NodeType parentType,
  unsigned int parentID, const boost::shared_ptr<DAQMapping>& mapping,
  const boost::shared_ptr<AnalysisMask>& mask)
{
#ifdef DEBUG
  printf(">> DAQMappingSourceXML::ParseTreeRP(%s, %u, %u)\n", XMLString::transcode(parent->getNodeName()),
    parentType, parentID);
#endif
  ...
  for (unsigned int i = 0; i < children->getLength(); i++) {
	...
	LogPrint("DAQMappingSourceXML") << ">> DAQMappingSourceXML::ParseTreeRP > Warning: tag `" << tagRPPot
					<< "' found in global scope, assuming station ID = 12.";
	parentID = 12;
      } else
        throw cms::Exception("DAQMappingSourceXML") << "Node " << XMLString::transcode(n->getNodeName())
    ...
    if (!id_set && type != nTriggerVFAT)
      throw cms::Exception("DAQMappingSourceXML::ParseTreeRP") << "id not given for element `"
       << XMLString::transcode(n->getNodeName()) << "'" << endl;

    if (!hw_id_set && type == nChip && pType == pMapping)
      throw cms::Exception("DAQMappingSourceXML::ParseTreeRP") << "hw_id not given for element `"
       << XMLString::transcode(n->getNodeName()) << "'" << endl;

    if (type == nRPPlane && id > 9)
      throw cms::Exception("DAQMappingSourceXML::ParseTreeRP") <<
    ...
    if (pType == pMapping && (type == nChip || type == nTriggerVFAT)) {
      const FramePosition &framepos = ChipFramePosition(n);
      VFATInfo vfatInfo;
      vfatInfo.hwID = hw_id;
...
void DAQMappingSourceXML::ParseTreeT2(ParseType pType, xercesc::DOMNode * parent, NodeType parentType,
  unsigned int parentID, const boost::shared_ptr<DAQMapping>& data,
  const boost::shared_ptr<AnalysisMask>& mask)
{
  DOMNodeList *children = parent->getChildNodes();
...
#ifdef DEBUG  
          printf("DAQMappingSourceXML: attribcounter_t2planedescript: %i \n",attribcounter_t2planedescript);
#endif

          if (attribcounter_t2planedescript>=25) {
            edm::LogVerbatim("DAQMappingSourceXML") << "T2-Plane attribute utilezed for parentID: position+info from parent ";
            ...
            if(testHS!=position_t2) {
              edm::LogPrint("DAQMappingSourceXML") <<"T2 Xml inconsistence in pl-pls attributes and position. Only 'position attribute' taken ";
              testHS=position_t2;
            }
            ...
            if (ID_t2!=(int)test_position_t2)
              edm::LogPrint("DAQMappingSourceXML") <<"T2 Xml inconsistence in plane attributes and xml parents structure. Plane attributes ignored";

          } else {
            // Case where arm-ht-pl-pls are NOT specified
            edm::LogVerbatim("DAQMappingSourceXML")<<"T2 Plane have parentID: "<<parentID<<" for its VFATs. Plane Position read: "<<position_t2;
          ...
          if (attribcounter_t2planedescript>=1) {
            ...
            if(attribcounter_t2planedescript>=5) {
              ...
              if ((int)parentID!=ID_t2) {
                edm::LogPrint("DAQMappingSourceXML") <<"T2 Inconsistence between plane 'id' and position from attributes. Id ignored";
                edm::LogPrint("DAQMappingSourceXML") <<" T2-Parent = "<<parentID;
              }
            } else {
              toaddForParentID=ID_t2;
              edm::LogVerbatim("DAQMappingSourceXML")<<" Number of T2 plane attributes: "<< attribcounter_t2planedescript<<" T2-Plane attribute utilezed for parentID: plane 'id' only";
            }
          } else {
//            position_t2Set = false;
            edm::LogProblem ("DAQMappingSourceXML") << "T2 plane not enough specified from its attribute!";
          }
        }
      }
      ...
      // content control
      if (idSet_t2 == false) {
        throw cms::Exception("DAQMappingSourceXML::ParseTree") << "ID_t2 not given for element `" << XMLString::transcode(n->getNodeName()) << "'" << endl;
        edm::LogProblem ("DAQMappingSourceXML") <<"ID_t2 not given for element `"<<XMLString::transcode(n->getNodeName()) << "'";
      }

      if (type == nChip && !hw_id_set)
        throw cms::Exception("DAQMappingSourceXML::ParseTree") << "hw_id not given for VFAT id `" <<
          ID_t2 << "'" << endl;

      if (type == nT2Det && position_t2 > 39) {
        throw cms::Exception("DAQMappingSourceXML::ParseTree") << "Plane position_t2 range from 0 to 39. position_t2 = " << position_t2 << " is invalid." << endl;
        edm::LogProblem ("DAQMappingSourceXML") <<"Plane position_t2 range from 0 to 39. position_t2 = "<<position_t2<< " is invalid.";
      }
    }
...
#ifdef DEBUG
          printf("DAQMappingSourceXML Found T2 special Vfat ChId-SLink-Symb  0x%x - %i - %i \n",
...
void DAQMappingSourceXML::ParseTreeT1(ParseType pType, xercesc::DOMNode * parent, NodeType parentType,
  unsigned int parentID, const boost::shared_ptr<DAQMapping>& mapping,
  const boost::shared_ptr<AnalysisMask>& mask, unsigned int T1Arm, unsigned int T1Plane, unsigned int T1CSC)
{
  ...
  for (unsigned int i = 0; i < children->getLength(); i++) {
      ...
      if (type == nT1Arm) {
        if (!strcmp(XMLString::transcode(a->getNodeName()), "id")) {
          ...
          if (id_arm != 0 && id_arm != 1) {
            throw cms::Exception("DAQMappingSourceXML::ParseTree") << "T1 id_arm neither 0 nor 1. Problem parsing XML file." << XMLString::transcode(n->getNodeName()) << endl;
            edm::LogProblem ("DAQMappingSourceXML") <<"T1 id_arm neither 0 nor 1. Problem parsing XML file."<<XMLString::transcode(n->getNodeName()) ;
          }
        ...
        if (!strcmp(XMLString::transcode(a->getNodeName()), "polarity")) {
          if (!strcmp(XMLString::transcode(a->getNodeValue()),"a")) {
            ...
            if (!strcmp(XMLString::transcode(a->getNodeValue()),"c")) {
              ...
              throw cms::Exception("DAQMappingSourceXML::ParseTree") << "T1: Neither anode nor cathode vfat : " << XMLString::transcode(n->getNodeName()) << endl;
              edm::LogProblem ("DAQMappingSourceXML") <<"T1: Neither anode nor cathode vfat : "<<XMLString::transcode(n->getNodeName());
            }
        }
    ...
    if (idSet_t1==false){
      throw cms::Exception("DAQMappingSourceXML::ParseTree") << "ID_t1 not given for element `" << XMLString::transcode(n->getNodeName()) << "'" << endl;
      edm::LogProblem ("DAQMappingSourceXML") <<"ID_t1 not given for element `"<<XMLString::transcode(n->getNodeName()) << "'";
    }

    if (type == nChip && !hw_id_set)
      throw cms::Exception("DAQMappingSourceXML::ParseTreeT1") <<
    ...
    if (type == nChip) {
      ...
      if (!tagT1CSC.compare(XMLString::transcode((n->getParentNode()->getNodeName())))) {
        ...
        throw cms::Exception("DAQMappingSourceXML::ParseTree") << "T1 has no special vfat `" << XMLString::transcode(n->getNodeName()) << "'" << endl;
        edm::LogProblem ("DAQMappingSourceXML") <<"T1 has no special vfat `"<<XMLString::transcode(n->getNodeName()) << "'";
      }
...
FramePosition DAQMappingSourceXML::ChipFramePosition(xercesc::DOMNode *chipnode)
{
  FramePosition fp;
  ...
  for (unsigned int j = 0; j < attr->getLength(); j++) {
    ...
    if (fp.SetXMLAttribute(XMLString::transcode(a->getNodeName()), XMLString::transcode(a->getNodeValue()), attributeFlag) > 1)
      throw cms::Exception("DAQMappingSourceXML") <<
...
DAQMappingSourceXML::NodeType DAQMappingSourceXML::GetNodeType(xercesc::DOMNode *n)
{
  // common node types
  ...
  throw cms::Exception("DAQMappingSourceXML::GetNodeType") << "Unknown tag `"
    << XMLString::transcode(n->getNodeName()) << "'.\n";
}

//----------------------------------------------------------------------------------------------------

void DAQMappingSourceXML::GetChannels(xercesc::DOMNode *n, set<unsigned char> &channels)
{
  ...
  for (unsigned int i = 0; i < children->getLength(); i++) {
    ...
    if (!idSet)
      throw cms::Exception("DAQMappingSourceXML::GetChannels") <<
        "Channel tags must have an `id' attribute.";
  }
}


DEFINE_FWK_EVENTSETUP_SOURCE(DAQMappingSourceXML);
