...
//
PPSTrackerOrganization :: PPSTrackerOrganization() : // _needUpdateUnitID(false),
					      ...
					      //	_currentObjectType(Undefined) 
{
  ...
  edm::LogInfo("PPSSim") << "Creating PPSTrackerOrganization";
...
PPSTrackerOrganization :: ~PPSTrackerOrganization() {
}
...
uint32_t PPSTrackerOrganization :: GetUnitID(const G4Step* aStep) const {
  return const_cast<PPSTrackerOrganization *>(this)->GetUnitID(aStep);
}


uint32_t PPSTrackerOrganization :: GetUnitID(const G4Step* aStep) {

//  std::cout << " INSIDE PPSTrackerOrganization :: GetUnitID " << std::endl;
...
/*
    ...
    std::cout<< " ***** WARNING TO BE FIXED: using  RP_box_secondary_vacuum to select Arm in PPSTrackerOrganization.cc. " <<std::endl; 
*/
...
//    _currentRP = 0; 
//    std::cout<< " **** WARNING TO BE FIXED: using _currentRP = 0 in PPSTrackerOrganization.cc " <<std::endl; 
...
edm::LogInfo("PPSSim") <<_currentArm << " " << _currentRP << " " << _currentPlane;
 ...
 PPSTrackerDetId id(_currentArm,_currentStation,_currentRP,_currentPlane);
